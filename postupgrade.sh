...
cp -v -r /tmp/uploads/$PTEMPDIR\_upgrade/config/$PDIR/* $PCONFIG/
echo "<INFO> Copy back existing log files..."
cp -v -r /tmp/uploads/$PTEMPDIR\_upgrade/log/$PDIR/* $PLOG/
echo "<INFO> Remove temporary folders..."
rm -r /tmp/uploads/$PTEMPDIR\_upgrade

echo "<INFO> installing latest version of \"python-solaredge-interface\" for python3..."
pip3 install --user --upgrade solaredge-interface
